<template>
  <el-tab-pane label="月结薪资计算" name="monthly">
    <!-- 工时汇总提示 -->
    <el-alert
    ...
    <el-card>
      <el-table v-loading="monthlyLoading" :data="monthlyRows" stripe>
        ...
        <el-table-column prop="status" label="状态" width="100">
          <template #default="{ row }">
            <el-tag :type="!row.status ? 'warning' : getStatusType(row.status)">
              {{ !row.status ? '待核算' : getStatusText(row.status) }}
            </el-tag>
          </template>
        </el-table-column>
...
async function handleBatchApproveCalculatedMonthly() {
  ...
  try {
    ...
    if (failCount > 0) {
      ElMessage.warning(`一键审核完成：成功 ${successCount} 条，失败 ${failCount} 条`);
    } else {
      ElMessage.success(`一键审核完成：成功 ${successCount} 条`);
    }
    await loadMonthlyData();
  } catch (err: any) {
    ElMessage.error(err?.message || '一键审核失败');
  } finally {
...
async function handleExportMonthly() {
  ...
  const rows = (exportResult.list || []).map((item) => ({
    ...
    状态: item.status ? getStatusText(item.status) : '待核算'
  }));
