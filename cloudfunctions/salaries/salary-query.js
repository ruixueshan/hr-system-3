...
async function getEmployeeSalaries(options = {}) {
  ...
  try {
    ...
    if (empByUserId.data?.length) {
      empByUserId.data.forEach(e => {
        if (e._id && !employeeIds.includes(e._id)) {
          employeeIds.push(e._id);
        }
      });
      console.log('[salary-query] 通过 user_id 查询到员工:', empByUserId.data.length, '人');
    }
  } catch (err) {
  ...
  if (user.phone) {
    try {
      ...
      if (empByPhone.data?.length) {
        empByPhone.data.forEach(e => {
          if (e._id && !employeeIds.includes(e._id)) {
            employeeIds.push(e._id);
          }
        });
        console.log('[salary-query] 通过 phone 查询到员工:', empByPhone.data.length, '人');
      }
    } catch (err) {
