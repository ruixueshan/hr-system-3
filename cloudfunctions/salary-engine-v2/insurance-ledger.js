/**
 * insurance-ledger.js (v2) - 保险台账管理
 * 改用本地 insurance-v2 (已经引用 common/date-utils)
 */
...
function buildLedgerPayload(params = {}) {
  const {
    employee_id, company_id, insurance_month,
    settlement_mode = 'monthly', join_date, leave_date,
    rate_plan_id, insurance_daily_deduct = 0,
    insurance_monthly_deduct = 0, obligation_amount, remark = ''
  } = params;
  ...
  const calculatedObligation = obligation_amount !== undefined
    ...
    : calculateMonthlyInsuranceObligation({
      year, month, joinDate: join_date, leaveDate: leave_date,
...
async function findLedger(collectionAccessor, params = {}) {
  ...
  const res = await collectionAccessor.collection('salary_insurance_ledgers')
    .where({ employee_id, company_id, insurance_month: normalizeYearMonth(insurance_month) })
    .limit(1)
    .get();
...
async function ensureInsuranceLedger(params = {}, collectionAccessor = db) {
  ...
  const addRes = await collectionAccessor.collection('salary_insurance_ledgers').add({ data: nowData });
  return {
    ledger: { _id: addRes._id || addRes.id, ...payload },
    created: true
  };
}

async function listInsuranceLedgers(params = {}, collectionAccessor = db) {
  const { employee_id, company_id, due_before_or_equal, status_list } = params;
  const conditions = {};
...
async function applyInsuranceDeduction(params = {}, collectionAccessor = db) {
  const { ledger_id, source_type, source_id = '', deduct_amount, pay_date, remark = '', created_by = '' } = params;
  if (!ledger_id || !source_type) throw new Error('缺少 ledger_id/source_type');

  const ledger = await getLedgerById(ledger_id, collectionAccessor);
  if (!ledger) throw new Error('保险台账不存在');
  if (ledger.status === 'cancelled') throw new Error('已取消的台账不能扣减');

  const requestAmount = roundMoney(Number(deduct_amount || 0));
  if (requestAmount <= 0) return { ledger, deduction: null, applied_amount: 0 };
  ...
  if (appliedAmount <= 0) return { ledger, deduction: null, applied_amount: 0 };
  ...
  const deductionPayload = {
    ledger_id, employee_id: ledger.employee_id, company_id: ledger.company_id,
    insurance_month: ledger.insurance_month, source_type, source_id,
    deduct_amount: appliedAmount, pay_date: pay_date || '', remark, created_by,
    created_at: collectionAccessor.serverDate ? collectionAccessor.serverDate() : db.serverDate()
  };

  const deductionRes = await collectionAccessor.collection('salary_insurance_deductions').add({ data: deductionPayload });
  await collectionAccessor.collection('salary_insurance_ledgers').doc(ledger_id).update({
    data: {
      deducted_amount: nextDeductedAmount, remaining_amount: nextRemainingAmount,
      status: nextStatus, last_deduct_source_type: source_type,
  ...
  return {
    ledger: {
      ...ledger, deducted_amount: nextDeductedAmount, remaining_amount: nextRemainingAmount,
      status: nextStatus, last_deduct_source_type: source_type, last_deduct_source_id: source_id
    },
    deduction: { _id: deductionRes._id || deductionRes.id, ...deductionPayload },
...
async function handleApply(data, operator) {
  const result = await applyInsuranceDeduction({
    ...data, created_by: data.created_by || operator?.uid || ''
  });
...
exports.handleInsuranceLedger = async (data = {}, operator = {}) => {
  ...
  switch (action) {
    case 'ensure': return handleEnsure(data, operator);
    case 'list': return handleList(data, operator);
    case 'get': return handleGet(data, operator);
    case 'apply': return handleApply(data, operator);
    default: return error(400, '未知保险台账操作');
  }
};

module.exports = {
  normalizeYearMonth, getNextYearMonth, compareYearMonth,
  buildLedgerKey, getFirstDueEventMonth, buildLedgerPayload,
  ensureInsuranceLedger, listInsuranceLedgers, getLedgerById,
  applyInsuranceDeduction, handleInsuranceLedger: exports.handleInsuranceLedger
};
