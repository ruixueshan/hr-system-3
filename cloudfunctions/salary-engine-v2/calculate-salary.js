...
const { roundMoney, calculateMonthlyInsuranceObligation, buildInsuranceShadowSnapshot } = require('./insurance-v2');
const {
...
exports.calculateSalary = async (params, operator) => {
  ...
  try {
    ...
    // ── 优化 2: 并行化查询 ──────────────────────────────
    // 第一批并行: employee + employee_companies + insurance config (三者无依赖)
    const [employeeDoc, ecRes, insuranceConfig] = await Promise.all([
      ...
      db.collection('employee_companies')
        ...
        .limit(1)
        .get(),
      getSalaryInsuranceV2RuntimeConfig(db)
    ]);

    if (!employeeDoc.data) throw new Error('员工不存在');
    const employee = employeeDoc.data;
    const employeeCompany = ecRes.data?.[0] || null;
    ...
    const insuranceDeduct = calculateInsuranceDeduct(
      plan,
      summary,
      year,
      month,
      employeeCompany?.join_date || summary?.join_date,
      employeeCompany?.leave_date || summary?.leave_date
    );
    const insuranceShadow = buildInsuranceShadowSnapshot({
      settlementMode, year, month,
    ...
    if (insuranceV2ShadowEnabled || insuranceV2Enabled) {
      console.log('[salary-insurance-v2]', JSON.stringify({
        ...
        employee_id, company_id, settlement_mode: settlementMode,
    ...
    if (settlementMode === 'monthly') {
      ...
      transactionMonthlyPayload = {
        ...
        company_name: summary.company_name || employee.company_name || employeeCompany?.company_name || '',
      ...
      salaryData = {
        ...
        job_id: employee.job_id || '', job_name: employee.job_name || '',
        ...
        hourly_rate: hourlyRate, regular_pay: basePay,
        overtime_pay: 0, base_pay: basePay,
        night_allowance: nightAllowance, insurance_deduct: insuranceDeduct,
        deductions: insuranceDeduct, gross_pay: grossPay,
        net_pay: netPay, total_amount: netPay,
        status: 'calculated',
        details: JSON.stringify({
          settlement_mode: 'daily', salaryDetails,
          night_hours: nightHours, night_days: nightDays,
          rate_plan_id: plan?._id || '', insurance_deduct: insuranceDeduct
        }),
    ...
    await db.runTransaction(async (transaction) => {
      ...
      if (settlementMode === 'monthly') {
        ...
        // 强制使用V2保险处理
        console.log(`[calculate-salary] 使用 V2 保险处理 ${employee_id}(${employee.name})`);
