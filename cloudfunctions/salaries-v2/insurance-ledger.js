...
function buildLedgerPayload(params = {}) {
  ...
  const calculatedObligation = obligation_amount !== undefined
    ...
    : isInsuranceMonthInV2Scope(normalizedMonth) ? calculateMonthlyInsuranceObligation({
      year,
      month,
      joinDate: join_date,
...
async function ensureInsuranceLedger(params = {}, collectionAccessor = db) {
  ...
  if (!isInsuranceMonthInV2Scope(payload.insurance_month)) {
    return {
      ledger: {
        _id: '',
        ...payload,
        remark: payload.remark || `保险V2从${SALARY_INSURANCE_V2_START_MONTH}起算，早期月份不生成台账`
      },
      created: false,
      skipped: true
    };
...
async function listInsuranceLedgers(params = {}, collectionAccessor = db) {
  ...
  return (res.data || []).filter((item) => {
    if (!isInsuranceMonthInV2Scope(item.insurance_month)) return false;
...
async function applyInsuranceDeduction(params = {}, collectionAccessor = db) {
  ...
  if (!isInsuranceMonthInV2Scope(ledger.insurance_month)) {
    return {
      ledger,
      deduction: null,
      applied_amount: 0
